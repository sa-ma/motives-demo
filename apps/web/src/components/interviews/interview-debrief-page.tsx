...
import type {
  InterviewDebriefEvidenceItem,
  InterviewDebriefModel,
  InterviewDebriefReasoningRow,
  InterviewDebriefTranscriptRow,
} from "@/components/interviews/interview-debrief.mock";
...
function TranscriptPanel({ debrief }: { debrief: InterviewDebriefModel }) {
  ...
  const [didApplyDesktopDefault, setDidApplyDesktopDefault] = useState(false);
  ...
  useEffect(() => {
    if (!didApplyDesktopDefault && isDesktop && defaultEvidenceId) {
      setSelectedEvidenceId(defaultEvidenceId);
      setDidApplyDesktopDefault(true);
    }
  }, [defaultEvidenceId, didApplyDesktopDefault, isDesktop]);
          ...
          {debrief.transcript.map((row, index) => {
            ...
            return (
              <TranscriptRow
                ...
                row={row}
                evidence={evidence}
                active={row.evidenceId === selectedEvidenceId}
